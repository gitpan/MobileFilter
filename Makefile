# This Makefile is for the MobileFilter::WURFLFilter extension to perl.
#
...
#     NAME => q[MobileFilter::WURFLFilter]
#     PREREQ_PM => { Apache2::Const=>q[0], CGI::Cookie=>q[0], Text::LevenshteinXS=>q[0], Apache2::Filter=>q[0], Apache2::RequestRec=>q[0], Apache2::RequestUtil=>q[0], APR::Table=>q[0], LWP::Simple=>q[0] }
...
NAME_SYM = MobileFilter_WURFLFilter
VERSION = 
...
MAKE = make
FULLEXT = MobileFilter/WURFLFilter
BASEEXT = WURFLFilter
PARENT_NAME = MobileFilter
DLBASE = $(BASEEXT)
VERSION_FROM = 
OBJECT = 
LDFROM = $(OBJECT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = WURFLFilter.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/MobileFilter
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/MobileFilter
...
TO_INST_PM = WURFLFilter.pm

PM_TO_BLIB = WURFLFilter.pm \
	$(INST_LIB)/MobileFilter/WURFLFilter.pm
...
DISTNAME = MobileFilter-WURFLFilter
DISTVNAME = MobileFilter-WURFLFilter-
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  WURFLFilter.pm $(INST_MAN3DIR)/MobileFilter::WURFLFilter.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:               MobileFilter-WURFLFilter' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '    Text::LevenshteinXS:  0' >> META_new.yml
...
ppd :
	...
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="APR-Table" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Apache2-Const" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Apache2-Filter" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Apache2-RequestRec" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Apache2-RequestUtil" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	...
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Text-LevenshteinXS" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' -- \
	  WURFLFilter.pm $(INST_LIB)/MobileFilter/WURFLFilter.pm 
